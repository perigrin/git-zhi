// ABOUTME: Main updater orchestration for git-zhi self-updater.
// ABOUTME: Coordinates version checking, downloading, and atomic replacement with recovery.
...
// validateDownloadedBinary checks that the file at path exists, is a regular file, and is at least 1 MB.
func validateDownloadedBinary(path string) error {
	info, err := os.Stat(path)
	if err != nil {
		return fmt.Errorf("downloaded binary not found: %w", err)
	}
	if !info.Mode().IsRegular() {
		return fmt.Errorf("downloaded binary is not a regular file: %s", path)
	}
	const minSize = 1024 * 1024 // 1 MB
	if info.Size() < minSize {
		return fmt.Errorf("downloaded binary is too small (%d bytes, minimum %d)", info.Size(), minSize)
	}
	return nil
}
...
func (u *Updater) PerformUpdate(opts *UpdateOptions) (*UpdateResult, error) {
	...
	tempDir, err := os.MkdirTemp("", "git-zhi-update-*")
	if err != nil {
