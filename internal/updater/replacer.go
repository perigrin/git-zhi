// ABOUTME: Atomic binary replacement with safety checks and rollback.
// ABOUTME: Handles installation method detection and platform-specific file operations.
...
// BinaryReplacer handles atomic replacement of the git-zhi binary.
...
type ReplacementOptions struct {
	CurrentPath    string // Path to current git-zhi binary
...
func (r *BinaryReplacer) checkRunningProcess(currentPath string) error {
	...
	if execPath == currentPath {
		...
		// On Unix-like systems, this is generally safe — the current process
		// will continue running the old binary until it exits.
	}
...
func (r *BinaryReplacer) validateNewBinary(newPath string) error {
	...
	minSize := int64(1024 * 1024)       // 1 MB minimum
	maxSize := int64(100 * 1024 * 1024) // 100 MB maximum
...
func (r *BinaryReplacer) copyPermissions(src, dst string) error {
	...
	if runtime.GOOS != "windows" {
		if err := r.copyOwnership(src, dst); err != nil {
			// Don't fail on ownership copy errors — it may not be possible
...
// GetCurrentBinaryPath returns the path of the currently running git-zhi binary.
...
// DetectInstallationMethod determines how git-zhi was installed.
...
// InstallationMethod represents how git-zhi was installed.
type InstallationMethod int

const (
	InstallationBinary        InstallationMethod = iota
	InstallationHomebrew                         // brew
...
// String returns a human-readable name for the installation method.
...
// CanSelfUpdate returns whether git-zhi can self-update given the installation method.
func (i InstallationMethod) CanSelfUpdate() bool {
	switch i {
	case InstallationBinary:
		return true
	case InstallationHomebrew:
		return false // Use 'brew upgrade git-zhi' instead.
	case InstallationSystemPackage, InstallationAPT, InstallationYum, InstallationDNF, InstallationPacman:
		return false // Use system package manager.
	case InstallationSnap:
		return false // Use 'snap refresh git-zhi'.
	case InstallationFlatpak:
		return false // Use 'flatpak update git-zhi'.
	case InstallationChocolatey, InstallationScoop, InstallationWinget:
		return false // Use respective package manager.
	default:
		return false
	}
}
...
func (i InstallationMethod) GetUpdateInstructions() string {
	switch i {
	case InstallationBinary:
		return "Use 'git zhi update' to update to the latest version"
	case InstallationHomebrew:
		return "Use 'brew upgrade git-zhi' to update via Homebrew"
	case InstallationSystemPackage:
		return "Use your system package manager to update git-zhi"
	case InstallationAPT:
		return "Use 'sudo apt update && sudo apt upgrade git-zhi' to update via APT"
	case InstallationYum:
		return "Use 'sudo yum update git-zhi' to update via YUM"
	case InstallationDNF:
		return "Use 'sudo dnf update git-zhi' to update via DNF"
	case InstallationPacman:
		return "Use 'sudo pacman -Syu git-zhi' to update via Pacman"
	case InstallationSnap:
		return "Use 'sudo snap refresh git-zhi' to update via Snap"
	case InstallationFlatpak:
		return "Use 'flatpak update git-zhi' to update via Flatpak"
	case InstallationChocolatey:
		return "Use 'choco upgrade git-zhi' to update via Chocolatey"
	case InstallationScoop:
		return "Use 'scoop update git-zhi' to update via Scoop"
	case InstallationWinget:
		return "Use 'winget upgrade git-zhi' to update via Winget"
	default:
