// ABOUTME: Rollback functionality for failed binary updates.
// ABOUTME: Handles automatic and manual rollback with backup validation.
...
// NewRollbackManagerWithDir creates a rollback manager that stores backups in the given directory.
...
func (rm *RollbackManager) ValidateRollbackTarget(targetPath string) error {
	...
	tempFile, err := os.CreateTemp(targetDir, ".git-zhi-rollback-test-*")
	if err != nil {
...
// Currently a no-op — the backup is preserved for forensic purposes.
