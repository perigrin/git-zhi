// ABOUTME: Multi-strategy recovery manager for git-zhi update failures.
// ABOUTME: Provides diagnosis, context creation, and ordered recovery strategies for various failure scenarios.
...
func (rm *RecoveryManager) getRecoveryStrategies(scenario RecoveryScenario) []*RecoveryStrategy {
	strategies := []*RecoveryStrategy{
		...
		},
		{
			...
			Description: "Retry download after network error",
			Priority:    1,
			Automatic:   true,
...
func (rm *RecoveryManager) downloadCompatibleVersion(ctx *RecoveryContext) error {
	...
	recoveryPath := ctx.TargetPath + ".recovery"
	...
	// Validate the downloaded binary: must exist and be > 1MB
	info, err := os.Stat(result.Path)
	if err != nil {
		os.Remove(result.Path)
		return fmt.Errorf("downloaded binary not found: %w", err)
	}
	const minBinaryBytes = 1024 * 1024
	if info.Size() < minBinaryBytes {
		os.Remove(result.Path)
		return fmt.Errorf("downloaded binary too small (%d bytes, need >%d)", info.Size(), minBinaryBytes)
	}

	// Make executable before installing
...
func (rm *RecoveryManager) checkDiskSpace(dir string) error {
	...
	tempFile, err := os.CreateTemp(dir, "git-zhi-space-check-*")
	if err != nil {
...
func (rm *RecoveryManager) FindCompatibleVersion() (*version.UpdateInfo, error) {
	...
	releases, err := client.GetReleases("perigrin", "git-zhi", true)
	if err != nil {
