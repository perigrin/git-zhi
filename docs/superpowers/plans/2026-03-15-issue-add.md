...
```bash
cd /home/perigrin/dev/git-zhi/.worktrees/issue-add
...
Run: `cd /home/perigrin/dev/git-zhi/.worktrees/issue-add && go test ./internal/storage/...`
...
Run: `cd /home/perigrin/dev/git-zhi/.worktrees/issue-add && go test ./internal/storage/... -v`
...
Run: `cd /home/perigrin/dev/git-zhi/.worktrees/issue-add && go test ./internal/issue/...`
...
Run: `cd /home/perigrin/dev/git-zhi/.worktrees/issue-add && go test ./internal/issue/... -v`
...
Run: `cd /home/perigrin/dev/git-zhi/.worktrees/issue-add && go test ./internal/config/... ./internal/milestone/...`
...
Run: `cd /home/perigrin/dev/git-zhi/.worktrees/issue-add && go test ./internal/cli/...`
...
func (a *App) EnsureInitialized() error {
	if a.Store.RefExists("refs/zhi/_/config") {
		return nil
	}
...
Run: `cd /home/perigrin/dev/git-zhi/.worktrees/issue-add && go test ./internal/cli/... -v`
Expected: PASS (12 tests — 8 root + 4 app)

- [ ] **Step 5: Verify full build**

Run: `cd /home/perigrin/dev/git-zhi/.worktrees/issue-add && go build ./...`
...
Run: `cd /home/perigrin/dev/git-zhi/.worktrees/issue-add && go test ./internal/cli/... -run TestIssueAdd`
...
Update `internal/cli/issue.go` — replace the stub `newIssueAddCommand` function and add `runIssueAdd`. Add necessary imports: `encoding/json`, `fmt`, `io`, `time`, `github.com/gofrs/uuid/v5`, `github.com/perigrin/git-zhi/internal/config`, `github.com/perigrin/git-zhi/internal/issue`.
...
func runIssueAdd(cmd *cobra.Command, args []string) error {
	...
	for _, iss := range created {
		...
		refPath := "refs/zhi/_/issues/" + iss.ID.String()
...
Run: `cd /home/perigrin/dev/git-zhi/.worktrees/issue-add && go vet ./... && go test ./...`
...
Run: `cd /home/perigrin/dev/git-zhi/.worktrees/issue-add && go vet ./... && go test -race ./...`
Expected: all clean, all pass.
...
cd $(mktemp -d)
git init
echo -e '---\ntitle: "Test issue"\n---\n\nHello world' | ../git-zhi issue add
../git-zhi issue add --format json <<'EOF'
---
title: "Batch one"
---
...
```bash
rm -f /home/perigrin/dev/git-zhi/.worktrees/issue-add/git-zhi
```

- [ ] **Step 4: Verify clean git status**

Run: `cd /home/perigrin/dev/git-zhi/.worktrees/issue-add && git status`
