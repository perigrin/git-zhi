...
## git-zhi-historian — Core Ecosystem Plugin
...
The historian reads `git log` directly and writes to `refs/zhi/` — both canonical issue refs and label indexes. It’s the only tool besides git-zhi core that writes to the ref store. This is necessary because it needs to create retrospective issues from historical data that predates zhi’s existence. The historian is not a one-time import tool — it maintains the chain as an ongoing bridge between git activity and zhi state.
...
Commits cluster when their score exceeds a configurable threshold. This handles cases that any single signal misses:
...
- **Both changed** → zhi wins and pushes outbound to tracker. The tracker’s change is overwritten. The conflict is logged so the user can review.

This prevents race conditions where a PM’s priority change in the tracker silently overwrites a developer’s urgency change in zhi.
...
Conflicts (zhi wins — modified since last sync):
  LOPS-145  Jira assignee changed to mjones, but zhi assigned to dev-c since last sync
            → keeping zhi value, pushing to Jira
```
...
**Project buffer.** The aggregate buffer protecting the initiative’s delivery date, computed from the cross-repo critical chain length and observed speed across repos.

**Feeding buffers.** At each cross-repo dependency junction, a feeding buffer absorbs delays from the feeder repo before they impact the downstream repo. Without feeding buffers, every repo’s forecast is independent and the project-level forecast ignores the reality that Repo B can’t start integration until Repo A delivers.

**Resource buffers.** When a shared developer needs to switch repos, the resource buffer provides early warning. If dev-a is finishing an issue in service-auth and is needed by microservices in two days, the resource buffer signals whether the handoff is on track or at risk.
...
### Phase 3: git-zhi-historian (core ecosystem)
